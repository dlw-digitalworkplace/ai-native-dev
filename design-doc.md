...
**In current scope, the coding agent is a warm in-session command (`/aind:implement <work-item-id>`) and stops at code-PR creation** (see `design-log.md` D24). It grounds from the merged plan and the `rules/*.md` files each task cites (the artifact-link contract of §5 Phase 1 / D17) plus the project's build/run skills, implements the task breakdown against the plan's Definition of done, runs the polish step in its own context, and opens the code PR — generating a readable code branch (`[type]/<work-item-id>-<short-name>`) that every later agent reaches *through the PR*, never by reconstructing the name (D17). **It authors no tests in this iteration** — both the cold test-writer (above) and the coder's own unit tests are deferred; the test-bearing design stays the target. The cold reviewer (Phase 4), the human merge gate, and the terminal `Implementation complete` write (D13) are the next steps, not part of this first build iteration.
